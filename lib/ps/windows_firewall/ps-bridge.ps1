...
function Get-PSFirewallRules {
    ...
    Show-NetFirewallRule | Where-Object { $_.DisplayName -in $filter } | ForEach-Object {
        ...
        $rules.Add(@{
                ...
                Service             = $sf.Service
                # Security Filter
                Security            = if (($secf.Authentication -eq 'Required') -and ($secf.Encryption -eq 'Required')) { 'Encryption' } elseif (($secf.Authentication -eq 'Required') -and ($secf.Encryption -eq 'NotRequired')) { 'Authentication' } else { 'NotRequired' }
...
function Get-NormalizedIpAddressRange {
    ...
    ### Sort IPs to avoid corrective loop ###
...
function Get-NormalizedValue {
    ...
    $normalize = @{
        ...
        "Security"            = { param($x); $x -replace "Authenticate", "Authentication" -replace "AuthEnc", "Encryption" }
    }
...
function Get-NormalizedKey {
    ...
    $keyNames = @{
        ...
        "RemoteUserGroup"     = "RemoteUser"
        "RemoteComputerGroup" = "RemoteMachine"
    }
...
function show {
    ...
    $missingLocalUser = New-Object System.Collections.ArrayList
    $rules = New-Object System.Collections.ArrayList
    $s0 = $(get-date)
    ...
    ForEach ($chunk in $($netshOutput -split "`r`n`r`n")) {
        ...
        if ($rule.get_count() -gt 0) {
            ...
            #Additional parsing required for LocalUser attribute (Doesn't exist in netsh output)
            if ($rule["Security"] -ne ("NotRequired")) {
                $missingLocalUser.Add($rule["Name"]) > $null
            }
        }
    }
    $s1 = $(get-date)
    ...
    if ($missingLocalUser.length) {
        # If there is missing local user, parse the rule with Get-PSFirewallRules
        $rules = $rules + (Get-PSFirewallRules $missingLocalUser)
    }
...
function create {
    ...
    if ($Security) {
        switch ($Security) {
            'Authentication' {$params.Add("Authentication", 'Required')}
            'Encryption' {
                $params.Add("Authentication", 'Required')
                $params.Add("Encryption", 'Required')
            }
        }
    }
    if ($LocalUser) {
